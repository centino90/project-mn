...
class Profile extends Model
{
  ...
  public function insertMultiple($vals): bool
  {
    ...
    foreach ($vals as $row) {
        ...
        $insertData['first_name' . $n] = $row[0];
        $insertData['middle_name' . $n] = $row[1];
        $insertData['last_name' . $n] = $row[2];
        $insertData['birthdate' . $n] = $row[3];
        $insertData['address' . $n] = $row[4];
        $insertData['contact_number' . $n] = $row[5];
        $insertData['gender' . $n] = $row[6];
        $insertData['fb_account_name' . $n] = $row[7];
        $insertData['prc_number' . $n] = $row[8];
        $insertData['prc_registration_date' . $n] = $row[9];
        $insertData['prc_expiration_date' . $n] = $row[10];
        $insertData['field_practice' . $n] = $row[11];
        $insertData['type_practice' . $n] = $row[12];
        $insertData['clinic_name' . $n] = $row[13];
        $insertData['clinic_street' . $n] = $row[14];
        $insertData['clinic_district' . $n] = $row[15];
        $insertData['clinic_city' . $n] = $row[16];
        $insertData['clinic_contact' . $n] = $row[17];
        $insertData['emergency_person_name' . $n] = $row[18];
        $insertData['emergency_address' . $n] = $row[19];
        $insertData['emergency_contact_number' . $n] = $row[20];
        $n++;
    }
