  ...
  <div class=" w-full space-y-8 lg:px-8 lg:py-10">
    ...
    <div class="flex flex-col lg:flex-row justify-center items-center gap-3">
      ...
      <?php elseif (isset($data['reason']) && $data['reason'] == 'passwordReset') : ?>
        <form action="<?php echo URLROOT ?>/users/forgotPassword" method="post" @submit="$refs.label.textContent = 'Please wait...'; $refs.submit.disabled = true;">
          <input type="email" name="email" hidden value="<?php echo $data['email']?>">

          <button type="submit" x-ref="submit" class="flex gap-3 items-center text-2xl rounded-lg border border-primary-500 py-3 px-5">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
            </svg>
