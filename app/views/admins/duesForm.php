  ...
  <div class="min-w-full px-4 lg:px-1">
    ...
    <header class="flex flex-wrap items-center justify-between gap-3 mb-10">
      <div class="flex-shrink-0">
        <span class="text-2xl font-bold">Add dues</span>
      </div>
    ...
    <form @submit.prevent x-ref="dues_form" x-show="!onImport" class="flex flex-col gap-y-8">
      <div x-bind="formGroup">
        <label for="user_id" class="form-label">PRC No. (required)</label>
            ...
            <div class="flex flex-col items-center relative z-0">
              <div class="w-full">
                <div class="mb-2 p-1 bg-white flex border border-secondary-200 rounded">
                  <input name="user_id" x-model="duesForm.prc_number" x-transition:leave="transition ease-in duration-100" x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0" @mousedown="open()" @keydown.enter.stop.prevent="selectOption()" @keydown.arrow-up.prevent="focusPrevOption()" @keydown.arrow-down.prevent="focusNextOption()" placeholder="Search profile name, email, or prc no."  autocomplete="off" class="p-1 px-2 appearance-none outline-none w-full text-secondary-800">
      ...
      <div x-bind="formGroup">
        <label x-bind="formGroup.formLabel" class="wrap">
          Channel (gcash, paymaya, etc.) (required)
        </label>
        <div x-bind="formGroup.inputContainer">
          <input type="number" x-model="duesForm.channel" x-bind="formGroup.formInput" name="channel">
      ...
      <div x-bind="formGroup">
        ...
        <div x-bind="formGroup.inputContainer">
          <input type="number" x-model="duesForm.or_number" x-bind="formGroup.formInput" name="or_number">
